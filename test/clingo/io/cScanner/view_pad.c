...
int main( void )
{
   ...
   expect_at_c_( sca->space == 10 );
   expect_at_c_( is_empty_c_( scanned_bytes_c_( sca ) ) );

   // ----------------------------------------------------------------- view pad
   cChars pad = view_pad_c( sca, '-' );
   expect_at_c_( chars_is_c( pad, "---" ) );
   expect_at_c_( sca->pos == 3 );
   expect_at_c_( sca->space == 7 );

   pad = view_pad_c( sca, 'a' );
   expect_at_c_( chars_is_c( pad, "a" ) );

   pad = view_pad_c( sca, 'b' );
   expect_at_c_( chars_is_c( pad, "bb" ) );
   expect_at_c_( sca->pos == 6 );
   expect_at_c_( sca->space == 4 );

   // --------------------------------------------- try to view not existing pad
   pad = view_pad_c( sca, 'X' );
   expect_at_c_( is_empty_c_( pad ) );
   expect_at_c_( sca->pos == 6 );
   expect_at_c_( sca->space == 4 );
