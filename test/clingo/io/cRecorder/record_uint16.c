...
int main( void )
{
   ...
   expect_at_c_( record_uint16_c( rec, 25386 ) );
   expect_at_c_( rec->pos == 2 );
   expect_at_c_( rec->space == 5 );
   expect_at_c_( *recVal == 25386 );

   // ----------------------------------------- write with a specific byte order
   cByte const* recBig = rec->mem;
   uint16_t const big = swap_uint16_to_c( 0x632a, c_BigEndian );
   expect_at_c_( record_uint16_c( rec, big ) );
   expect_at_c_( recBig[ 0 ] == 0x63 );
   expect_at_c_( recBig[ 1 ] == 0x2A );

   cByte const* recLittle = rec->mem;
   uint16_t const little = swap_uint16_to_c( 0x632a, c_LittleEndian );
   expect_at_c_( record_uint16_c( rec, little ) );
   expect_at_c_( recLittle[ 0 ] == 0x2A );
   expect_at_c_( recLittle[ 1 ] == 0x63 );

   // --------------------------------------- try tp record without enough space
   expect_at_c_( not record_uint16_c( rec, 1 ) );
