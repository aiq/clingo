...
int main( void )
{
   ...
   expect_at_c_( record_mem_c( rec, 1, &first ) );
   expect_at_c_( *firstPtr == 0x33 );

   uint16_t const* secondPtr = rec->mem;
   expect_at_c_( record_mem_c( rec, 2, &second ) );
   expect_at_c_( *secondPtr == 0x7777 );

   expect_at_c_( not record_mem_c( rec, 3, "abc" ) ); // to large
