...
int main( void )
{
   ...
   expect_at_c_( *get_from_byte_vec_c( vec, 0 ) == 0xBA );
   expect_at_c_( *get_from_byte_vec_c( vec, 3 ) == 0xEF );
   expect_at_c_( *get_from_byte_vec_c( vec, 5 ) == 0xDE );

   *get_var_from_byte_vec_c( vec, 3 ) = 0xBA;
   set_on_byte_vec_c( vec, 5, 0x11 );
   expect_at_c_( *get_from_byte_vec_c( vec, 3 ) == 0xBA );

   insert_into_byte_vec_c( vec, 5, 0xC0 );

   expect_at_c_( *get_from_byte_vec_c( vec, 6 ) == 0x11 );
   expect_at_c_( info->count == 7 );

   cRecorder* rec = &recorder_c_( 128 );
   write_byte_vec_c( rec, vec, "X//2" );

   expect_at_c_( recorded_is_c( rec, "BABE BEBA C0C0 11" ) );
