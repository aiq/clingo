...
Generates a function that allows to insert the elements from the source slice
into the destination slice.
The existing values will be moved, the overhang values at the end will be
removed.
...
The generated function returns 1 if the value is inserted, otherwise 0.
[source,c]
----
...
[source,c]
----
include::{xmpldir}/insert_slice.c[]
----
...
Generates a function that allows to insert a value into a slice.
The existing values will be moved, the overhang value at the end will be
removed.

FuncName:: {FuncName}
VarSliceType:: {SliceType1}
ValueType:: {ValueType1}
InsertType:: the type of the parameter
DoDeref:: can be "*" to dereference the pointer of the InsertType

The generated function returns the number of elements that are inserted into the
dst slice, otherwise 0.
[source,c]
----
