= cScanner
:xmpldir: ../../../test/clingo/io/cScanner
...
cScanMarker allows to trace how much byte of the memory block are scanned.
...
Creates a marker for the scanner Sca.
...
int64_t trace_scan_c( cScanMarker sm[static 1], bool full );
----

The full value tells to return the number of bytes that are scanned since
the creation of the marker(true) or since the last call of the function(false).
...
cBytes traced_bytes_c( cScanMarker sm[static 1], bool full );
----

The full value tells to return the bytes that are scanned since the creation of
the marker(true) or since the last call of the function(false).
...
cBytes traced_chars_c( cScanMarker sm[static 1], bool full );
----

The full value tells to return the chars that are scanned since the creation of
the marker(true) or since the last call of the function(false).

==== undo_scan_c
[source,c]
----
bool undo_scan_c( cScanMarker sm[static 1] );
----

Moves the scanner back to the creation of the marker.

==== undo_scan_error_c
[source,c]
----
bool undo_scan_error_c( cScanMarker sm[static 1], int err );
----

Moves the scanner back to the creation of the marker, sets the err value on the
scanner and returns allways false.