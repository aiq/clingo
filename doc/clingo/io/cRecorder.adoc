...
[id='dyn_heap_recorder_c_']
==== dyn_heap_recorder_c_
[source,c]
----
...
[id='fix_heap_recorder_c_']
==== fix_heap_recorder_c_
[source,c]
----
...
[source,c]
----
bool exrecord_bytes_c( cRecorder rec[static 1], cBytes bytes );
----

Writes the byte values of the byte slice to the memory of the recorder.
Extends via reallocs the memory if more space is needed.
Returns true if all byte values can be written, otherwise false.

==== exrecord_chars_c
[source,c]
----
bool exrecord_chars_c( cRecorder rec[static 1], cChars chars );
----

Writes the char values of the char slice to the memory of the recorder.
Extends via reallocs the memory if more space is needed.
Returns true if all char values can be written, otherwise false.
...
[source,c]
----
bool exrecord_mem_c( cRecorder rec[static 1], int64_t len, void const* mem );
----
...
==== exrecord_terminated_c
[source,c]
----
bool exrecord_terminated_c( cRecorder rec[static 1], cChars chars );
----
